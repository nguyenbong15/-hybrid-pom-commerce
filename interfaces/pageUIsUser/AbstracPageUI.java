package pageUIsUser;

public class AbstracPageUI {
	public static final String CUSTOMER_INFO_LINK = "//div[@class='listbox']//a[text()='Customer info']";
	public static final String ADDRESSES_LINK = "//div[@class='listbox']//a[text()='Addresses']";
	public static final String ORDER_LINK = "//div[@class='listbox']//a[text()='Orders']";
	public static final String CHANGE_PASSWORD_LINK = "//div[@class='listbox']//a[text()='Change password']";
	public static final String MY_PRODUCT_REVIEW_LINK = "//div[@class='listbox']//a[text()='My product reviews']";
	public static final String DYNAMIC_LOCATOR = "//div[@class='listbox']//a[text()='%s']";
	public static final String SEARCH_LINK_FOOTER = "//ul[@class='list']//a[text()='Search']";
	public static final String COMPUTER_LINK="//ul[@class='top-menu notmobile']//li[1]//a[text()='Computers ']";
	public static final String DESKTOP_LINK="//ul[@class='top-menu notmobile']//a[text()='Desktops ']";
	public static final String TEXBOX_BY_ID="//input[@id='%s']";
	public static final String RADIO_BY_ID="//input[@id='%s']";
	public static final String CHECKBOX_BY_ID="//input[@id='%s']";
	public static final String ICON_LOADING = "//div[@id='ajaxBusy']/span";
	public static final String 	ID_PANEL = "//div[@id='%s']";
	public static final String 	ICON_HIDE_OR_SHOW = "//div[@id='%s']//button[@class='btn btn-tool']/i";
}
